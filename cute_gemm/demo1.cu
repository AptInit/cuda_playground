...
template <auto kernelTile, typename TLA, typename TLB, typename TLDst,
    typename PredFunc>
__device__ static void kernel_block(const size_t tileA_idx,
    const size_t tileB_idx, const size_t tile_idx, const TLA& tiledA,
...
template <dim3 block>
__global__ static void kernel_single_block(const size_t m, const size_t n,
    const size_t k, const float* A, const float* B, const float* C,
    float* dst) {
  ...
  constexpr auto blockTile =
      cute::Shape<cute::C<block.x>, cute::C<block.y>>{};  // tileN, tileM
  ...
  auto make_basis = [] __device__(const auto layout, const auto tiler) {
    ...
    return std::make_tuple(
        cute::zipped_divide(cute::make_layout(layout.shape(),
                                cute::make_stride(cute::_1{}, cute::_0{})),
            tiler),
        cute::zipped_divide(cute::make_layout(layout.shape(),
                                cute::make_stride(cute::_0{}, cute::_1{})),
            tiler));
  };
  ...
  for (size_t tileIdx = 0; tileIdx < cute::size<1>(tiledDst); ++tileIdx) {
    ...
    kernel_block<blockTile>(tileIdxA, tileIdxB, tileIdx, tiledA, tiledB,
        tiledDst, predicate, A, B, C, dst);
  }
}

template <unsigned grid_x, unsigned grid_y, unsigned grid_z, unsigned block_x,
    unsigned block_y, unsigned block_z>
__global__ static void kernel_grid(const size_t m, const size_t n,
    const size_t k, const float* A, const float* B, const float* C,
    float* dst) {
  ...
  for (size_t gridIdx = 0; gridIdx < cute::size<1>(grdTiledDst); ++gridIdx) {
    ...
    kernel_block<blockTiler>(tileIdxA, tileIdxB, tileIdx, blkTiledA, blkTiledB,
        blkTiledDst, predicate, A, B, C, dst);
  }
}
...
static void kernel_grid_caller(const size_t m, const size_t n, const size_t k,
    const float* A, const float* B, const float* C, float* dst) {
  kernel_grid<grid.x, grid.y, grid.z, block.x, block.y, block.z>
      <<<grid, block>>>(m, n, k, A, B, C, dst);
}

void gemm_f32_row_row_row_row_cuda(const size_t m, const size_t n,
    const size_t k, const float* A, const float* B, const float* C,
    float* dst) {
  ...
  // kernel_single_block<block><<<{1,1,1}, block>>>(m,n,k,matA.ptr(),
  // matB.ptr(), matC.ptr(), matD.ptr());
  kernel_grid_caller<grid, block>(
