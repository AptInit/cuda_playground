...
void gemm_f32_row_row_row_row(const size_t m, const size_t n, const size_t k,
  ...
  auto kernel = [&A, &B, &C, &dst, &tiledA, &tiledB, &tiledC, &tiledDst,
                    kernelTile]<typename PredFunc>(const auto tileA_idx,
                    const auto tileB_idx, const auto tile_idx,
                    PredFunc predicate) {
    ...
    {
      auto layoutTileA = cute::get<0>(
          cute::slice(cute::make_coord(cute::_, tileA_idx), tiledA));
      auto layoutTileB = cute::get<0>(
          cute::slice(cute::make_coord(cute::_, tileB_idx), tiledB));
      // Sanity check
      assert(cute::get<1>(layoutTileA.shape()) ==
             cute::get<0>(layoutTileB.shape()));
    }
    ...
    const auto dimK =
        cute::size<1>(cute::shape<0>(tiledA));  // K dimension size
