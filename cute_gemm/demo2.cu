...
template <auto kernelTile, size_t tileK, typename TLA, typename TLB, typename TLDst, typename PredFunc>
__device__ static void kernel_block(
    ...
    constexpr size_t tileN = cute::size<0>(kernelTile);
    constexpr size_t tileM = cute::size<1>(kernelTile);
    // static_assert(
    //     util::floor(tileK, std::lcm(tileN, tileM)) == tileK);
    ...
    for (size_t tileKIdx=0; tileKIdx<cntK;++tileKIdx) {
...
__global__ static void kernel_grid_v2(
    ...
    for (size_t gridIdx = 0; gridIdx < cute::size<1>(grdTiledDst); ++gridIdx) {
        ...
        constexpr unsigned int tileK = 96;//std::lcm(block.x, block.y);
...
void gemm_f32_rrrr_cuda_v2(
    ...
    kernel_grid_v2<grid, block><<<grid, block>>>(m,n,k,matA.ptr(), matB.ptr(), matC.ptr(), matD.ptr());
